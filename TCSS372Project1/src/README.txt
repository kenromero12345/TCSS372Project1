...
4. Memory: The MIPS simulator will have 100 memory spaces. The data segment will start at address 0x10010000 (represented internally as index 0-49), and the stack pointer will start at address 0x7FFFEFFC (represented internally as index 50-99) allocated for the stack. The global pointer starting address will be at 0x10008000 (this value does not change throughout the execution of the program). A graphical user interface will be used to show the state of each memory location. Each memory location will have 4 bytes, so each memory location will be incremented by 4. Instructions will start at address 0x00400000.
